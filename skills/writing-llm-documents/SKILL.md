...
This skill helps you write documents that a consumer can parse, prioritise,
...
You MUST apply this skill when the primary consumer of the document is
...
When a document contains binding technical requirements, you MUST apply
`normative-technical-writing` to their wording. You MAY use this skill
together with `writing-human-documents` when a document will be read by multiple
consumers. You MUST preserve the consumer-facing contract even when
...
- **MAY** permits you or the document consumer to take the action, but does
  not impose the action.

You MUST treat every `MUST` and `MUST NOT` instruction as binding.
You MUST NOT infer a hidden requirement from a heading, example, rationale,
comment, footnote, diagram, or conversational implication.

The document MUST use `normative-technical-writing` for its own
normative vocabulary and obligation wording.

## Document contract

For every consumer-facing document, you MUST define or explicitly mark as not
applicable:
...
You MUST NOT leave a contract item implicit when its absence could change
the consumer's behaviour. You MAY omit genuinely irrelevant detail, but
you MUST state when a contract item is not applicable if a reader could
reasonably expect it to exist.

## Authority and scope

You MUST establish the document's authority near the beginning. The
...
You MUST distinguish the document's own rules from rules imported from
another source. You MUST identify every external dependency that is
necessary to interpret a requirement.

You MUST state precedence when multiple sources, sections, or rules can
apply to the same situation. You MUST NOT rely on position, recency, or
the consumer's presumed common sense as an unstated conflict-resolution mechanism.

You MUST define the boundary of the document. The document MUST state
what it does not govern when a nearby responsibility could be confused with its
scope.

## Document shape

You MUST use a stable, recognisable structure and MAY adapt it when the
...
You MUST keep headings stable and descriptive. You MUST NOT use a
heading only for visual decoration. You MAY omit a section that is not
applicable, but you MUST either record that fact or make the omission
...
you MUST apply `normative-technical-writing`.

You MUST NOT hide two unrelated obligations inside one sentence merely
because they share a subject. You MUST split compound rules when each
part could be violated independently.

You MUST state conditions explicitly. Words such as "appropriate",
...
You MUST state the consequence of non-compliance when the consumer needs to
choose between continuing, asking a question, reporting an error, or refusing
the task.

## Definitions and terminology

You MUST define a term before using it normatively. Definitions MUST be
specific enough that two readers apply the term to the same case.

You MUST use one canonical term for one concept within a document.
You MUST NOT alternate between synonyms when the distinction could matter.
You MUST preserve the same capitalisation, spelling, and singular or
plural form for a canonical term.

You MUST replace vague references such as "this", "it", "the above", or
"the usual process" with the named actor, rule, section, or object. You
...
You MAY assign stable identifiers to definitions, requirements, inputs,
...
You MUST separate binding rules from material that explains or illustrates
those rules. You MUST use explicit labels such as:
...
You MUST label every example and non-example. Examples MUST demonstrate
the governing rules and MUST NOT silently add a narrower requirement.

You MUST NOT place a required instruction only in a comment, footnote,
diagram, image, alt text, example, or rationale section. Required behaviour MUST
appear in explicit normative prose.

If an example contains `MUST`, `MUST NOT`, or `MAY`, you MUST make clear
whether the terms are part of the document's rules or quoted content. You
MUST NOT allow an illustrative example to override a normative clause.

You MAY include rationale after a rule when it improves understanding.
...
You MUST distinguish absent input from an empty, zero, false, unknown, or
invalid value when those cases produce different behaviour.

You MUST NOT instruct the consumer to "fill in" unspecified values from an
unstated assumption. You MUST define a default, require clarification,
permit an explicit assumption, or require a failure response.

If input fields have dependencies, you MUST state the dependency and its
...
You MUST provide a schema, template, or concrete structural example when
the output shape is important. You MAY use JSON, YAML, tables, Markdown,
or another format, but you MUST identify the selected format and MUST NOT
mix incompatible formats without explaining the boundary.

You MUST distinguish content requirements from presentation preferences.
...
You MUST define how the consumer represents uncertainty, unavailable data,
and assumptions in its output. You MUST NOT require the consumer to invent
facts to complete an output.

## Processes and decision flows

When a task has an order, you MUST use numbered steps or an ASCII diagram
...
You MUST define retry limits, loop termination, escalation, and fallbacks
when any of them are possible. You MUST NOT use an unbounded instruction
such as "keep trying" without a stopping condition.

You MUST use plain ASCII diagrams for relationships, flows, boundaries, or
...
You MUST follow every diagram with a plain-language explanation. The
diagram MUST NOT be the only source of a required rule.

## Context and token efficiency

You MUST place high-priority scope, authority, and behavioural constraints
before supporting detail. You MUST NOT bury a required instruction in a
long preamble or at the end of unrelated context.

You MUST remove duplicated rules and keep one canonical statement for
...
You MUST keep the document self-contained enough for its declared scope.
If interpretation depends on another document, you MUST identify that
document and the required section or contract. You MUST NOT assume that a
conversation, tool result, file, or prior instruction is available unless the
document declares that dependency.

You MAY use progressive disclosure: a compact instruction summary first,
followed by definitions, rules, examples, and supporting detail. You MUST
...
You MUST use explicit placeholders for values the consumer must provide.
You MUST NOT use vague placeholders such as "insert details" or "fill in
as appropriate" when the required value or selection can be named.

## Ambiguity and conflicts

You MUST resolve contradictions before finalising the document. Two
...
When ambiguity cannot be removed, you MUST define the response. The
...
You MUST distinguish an unknown fact from a false fact. You MUST
define whether the consumer MAY proceed with an explicit assumption or MUST stop for
clarification.

You MUST state how conflicts between this document and external sources
are resolved. You MUST NOT rely on the consumer to infer authority from the
apparent confidence, length, or position of a source.

## Examples and non-examples

You MAY include examples when they reduce ambiguity or demonstrate the
...
`Invalid example`, `Input example`, or `Output example`. You MUST NOT
...
You MUST update examples when a normative rule changes. Examples MUST NOT
...
You MUST use Markdown headings to express document hierarchy. Heading
levels MUST progress logically, and headings MUST describe their content.

You MUST use lists for parallel requirements and numbered lists for
ordered procedures. You MUST use tables only when the rows share
comparable criteria.

You MUST put schemas, templates, literal output, and diagrams in fenced
code blocks. You MUST label a code fence with its language when a parser
or reader needs that information.

You MUST NOT hide normative text in styling, colour, indentation, or
layout. The document MUST remain interpretable as plain text.

You MUST use ASCII diagrams only. ASCII diagrams MUST have clear labels,
explicit direction, and a prose explanation. You MUST NOT use Mermaid,
...
- **The implied contract:** The document assumes the consumer knows an input, output,
  role, or domain rule. You MUST state the dependency or remove it.
- **The rule soup:** Several obligations appear in one long sentence. You
  MUST split them into atomic normative clauses.
- **The modal blur:** Guidance uses words such as "usually" or "prefer" without
  defining their force. You MUST replace them with `MUST`, `MUST NOT`, or
  `MAY`.
- **The example trap:** An example contains an extra restriction that is absent
  from the rules. You MUST label examples and MUST ensure they add no
  silent constraints.
- **The vague fallback:** The document says to handle an error appropriately.
  You MUST define the exact response, stopping condition, or escalation.
- **The context dependency:** The document relies on a conversation or external
  source that is not declared. You MUST state the dependency and required
  contract.
- **The conflict pair:** Two sections prescribe incompatible outcomes. You
  MUST resolve the conflict or state precedence.
- **The output vacuum:** The document describes a task but not the required
  result. You MUST define the output contract and validation conditions.
- **The repetition tax:** The same rule appears in several slightly different
  forms. You MUST retain one canonical rule and link to it elsewhere.
- **The specialist diagram:** A diagram requires a renderer or notation the
  consumer may not support. You MUST replace it with a plain ASCII diagram
