---
name: writing-prds
description: Guidance for authoring Product Requirements Documents (PRDs) for discrete features. Use when creating, updating, or reviewing a PRD in the project's requirements directory, or when a user asks for a product requirements doc, feature spec, or PRD.
---
...
A **Product Requirements Document (PRD)** specifies what a discrete feature must do, why it matters, and how users experience it — at the level of behaviour and contract, not implementation. A PRD is the bridge between the project's evergreen product context and the engineering work that realises it.

Use this skill when:

- A user asks for a "PRD", "product requirements doc", "feature spec", or "requirements document" for a discrete feature.
- You are creating or updating a file in the project's established requirements directory.
...
- Implementation plans — those follow the project's implementation-plan template.

## When to write a PRD vs an ADR vs an implementation plan

| Artifact | Question it answers | Where it lives |
|---|---|---|
| **PRD** | What should the feature do, and why? | the project's requirements directory |
| **ADR** | What architectural choice did we make, and why? | the project's ADR directory |
| **Implementation plan** | How do we build it, module by module? | the project's implementation-plan template |
...
1. **Understand the problem.** Discover and read the relevant evergreen product docs, existing code, and ADRs the feature touches. Do not draft blind or assume fixed filenames.
...
5. **Save with the next number.** Determine the next sequential `NNNN` by listing the project's established requirements directory, then write `NNNN-kebab-case-title.md`. Sequential numbering is mandatory even if earlier PRDs are superseded — gaps are expected and fine.
...
- **Directory:** the project's established requirements directory
- **Naming:** `NNNN-descriptive-title-with-dashes.md` (use the project's established document convention)
...
7. **Domain model diff:** Does §7 diff against the project's existing domain model rather than restating it?
...
- **Restating the domain model.** §7 should *diff* against the project's existing domain model, not copy it. Reference the existing model and describe only what changes.
...
- The first PRD written against this template in the project's requirements directory — use it as a reference for tone, section length, and how to handle a PRD that supersedes an ADR. Discover the directory rather than assuming its path.
